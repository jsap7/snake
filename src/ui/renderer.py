...
class Renderer:
    def __init__(self, screen, color_scheme="blue"):
        ...
        # Load Rubik font with medium weight
        font_path = os.path.join("src", "ui", "fonts", "Rubik-Medium.ttf")  # or Rubik-Bold.ttf for even thicker text
        try:
            self.score_font = pygame.font.Font(font_path, 42)
            self.ai_font = pygame.font.Font(font_path, 36)
            self.title_font = pygame.font.Font(font_path, 100)
            self.game_over_font = pygame.font.Font(font_path, 74)
            self.info_font = pygame.font.Font(font_path, 36)
        except:
            print("Failed to load Rubik font, using default")
            self.score_font = pygame.font.Font(None, 42)
            self.ai_font = pygame.font.Font(None, 36)
    ...
    def draw_score(self, score, is_ai_mode=False, ai_name=None):
        ...
        if is_ai_mode and ai_name:
            ai_text = self.ai_font.render(f'AI: {ai_name}', True, START_TEXT_COLOR)
            ai_rect = ai_text.get_rect(topright=(WINDOW_SIZE - 20, 20))
